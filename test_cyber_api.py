...
dash = test_endpoint('Cyber Dashboard Stats', 'http://localhost:8000/api/cyber/dashboard')
print(f"   Initial Security Status: {dash['security_status']}, Availability: {dash['system_availability']}%")

# 2. Test Asset Inventory
assets = test_endpoint('Cyber Assets', 'http://localhost:8000/api/cyber/assets')
print(f"   Found {len(assets['assets'])} critical infrastructure assets")

# 3. Start Ransomware Simulation
sim = test_endpoint('Start Ransomware Sim', 'http://localhost:8000/api/cyber/simulation/start', method='POST', data={
...
step2 = test_endpoint('Advance Step 2', 'http://localhost:8000/api/cyber/simulation/next-step', method='POST')
...
step3 = test_endpoint('Advance Step 3', 'http://localhost:8000/api/cyber/simulation/next-step', method='POST')
step4 = test_endpoint('Advance Step 4 (Isolation)', 'http://localhost:8000/api/cyber/simulation/next-step', method='POST')
...
events = test_endpoint('Cyber Event Log', 'http://localhost:8000/api/cyber/events')
print(f"   Logged {len(events['events'])} security events")

# 7. Test Incident Detail
incidents = test_endpoint('Cyber Incidents List', 'http://localhost:8000/api/cyber/incidents')
inc_id = incidents['incidents'][0]['id']
inc_detail = test_endpoint(f'Incident Detail ({inc_id})', f'http://localhost:8000/api/cyber/incidents/{inc_id}')
print(f"   Incident Detail: {inc_detail['incident']['title']} (Events: {len(inc_detail['incident']['timeline'])})")

# 8. Test Reset Demo
reset_res = test_endpoint('Reset Cyber Demo', 'http://localhost:8000/api/cyber/simulation/reset', method='POST')
...
inc_list = test_endpoint('Existing Incident List', 'http://localhost:8000/api/incidents')
